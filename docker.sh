...
install() {
    ...
    if [[ ! -f .env ]]; then
        ...
        curl -L https://raw.githubusercontent.com/vebyirdhani/AzuraCast-master/master/.env > .env
    fi

    if [[ ! -f azuracast.env ]]; then
        echo "Creating default AzuraCast settings file..."
        curl -L https://raw.githubusercontent.com/vebyirdhani/AzuraCast-master/master/azuracast.sample.env > azuracast.env
    fi

    if [[ ! -f docker-compose.yml ]]; then
        echo "Retrieving default docker-compose.yml file..."
        curl -L https://raw.githubusercontent.com/vebyirdhani/AzuraCast-master/master/docker-compose.sample.yml > docker-compose.yml
    fi
...
#
update-self() {
    curl -L https://raw.githubusercontent.com/vebyirdhani/AzuraCast-master/master/docker.sh > docker.sh
    chmod a+x docker.sh
